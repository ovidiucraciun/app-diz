...
//  CMD ["/usr/bin/java", "-jar", "-Dspring.profiles.active=default", "/opt/spring-cloud/lib/app-diz-0.0.1-SNAPSHOT.jar"]" > Dockerfile""")
    ...
    stage("Push image to ACR"){
        withCredentials([azureServicePrincipal('jenkins-ad')]){
           sh ("pwd && ls -al")
           sh ("az acr build --image dizertatie/diz-app:v4 --registry aksdizregistry --file Dockerfile .")
        }
    }
    stage("Deployment"){
        withCredentials([azureServicePrincipal('jenkins-ad')]){
           ...
           sh "(kubectl run diz-app-deploy --image=aksdizregistry.azurecr.io/dizertatie/diz-app:v4 --replicas=2 --port=8080)"
