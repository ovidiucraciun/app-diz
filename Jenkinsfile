...
//           sh "cp build/libs/* /mnt/csb3b6d9a4ea33ex4761xb9d"
           ...
           sh "curl -X PUT -d build/libs/* https://csb3b6d9a4ea33ex4761xb9d.file.core.windows.net/artifacts-storage"
//           sh "ls -la /mnt/csb3b6d9a4ea33ex4761xb9d/"
//           sh 'sudo azcopy copy "build/libs/*" "https://csb3b6d9a4ea33ex4761xb9d.file.core.windows.net/artifacts-storage"'
         }
      }
...
//  CMD ["/usr/bin/java", "-jar", "-Dspring.profiles.active=default", "/opt/spring-cloud/lib/app-diz-0.0.1-SNAPSHOT.jar"]" > Dockerfile""")
    ...
    stage("Push image to ACR"){
        withCredentials([azureServicePrincipal('jenkins-ad')]){
           sh ("pwd && ls -al")
           sh ("az acr build --image dizertatie/diz-app:v13 --registry aksdizregistry --file Dockerfile .")
        }
    }
    stage("Deployment"){
        withCredentials([azureServicePrincipal('jenkins-ad')]){
           ...
           sh "(kubectl run diz-app-deploy --image=aksdizregistry.azurecr.io/dizertatie/diz-app:v13 --replicas=2 --port=8080)"
        ...
        //   sh '(sed -i "s/diz-app-deploy..*/diz-app-deploy.${dns_var}/g" ingress/ingress-app-deploy.yaml)'
           sh "(kubectl apply -f ingress/ingress-app-deploy.yaml && sleep 1m)"
           sh "(kubectl get ingress > ip1.txt && tail -n 1 ip1.txt > ip2.txt && cat ip2.txt)"
           sh "(cat ip2.txt && grep -oP '(([0-9][0-9][0-9])|([0-9][0-9])|([0-9])).(([0-9][0-9][0-9])|([0-9][0-9])|([0-9])).(([0-9][0-9][0-9])|([0-9][0-9])|([0-9])).(([0-9][0-9][0-9])|([0-9][0-9])|([0-9]))' ip2.txt > ip.txt && cat ip.txt)"
           sh '(ip=$(cat ip.txt) && echo "\$ip" && az network dns record-set a add-record -g UTCN -z craciuno.com -n app -a "\$ip")'
        }
    }
